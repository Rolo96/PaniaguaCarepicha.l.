...
namespace GasStationPharmacy.Repositories
{
    ...
    public class RepositorioMedicamento
    {
        ...
        public static List<ObjGeneral> ConsultarMedicamento(string nombre)
        {
            //Lista que almacena los clientes
            var lista = new List<ObjGeneral>();
            //Query que consulta la base de datos
            var query = "SELECT [Prescripcion]" +
                " FROM[dbo].[MEDICAMENTO] WHERE Activo=1 AND Nombre= @nombre ";
            //Se ejecuta el query
            try
            {
                conexion.Close();
                ...
                using (var reader = comando.ExecuteReader())
                {
                    while (reader.Read())//Se agrega la informacion leida a la lista
                    {
                        ...
                        {
                            opcion = reader.GetBoolean(0).ToString()
                        });
                    }
                }
                conexion.Close();
